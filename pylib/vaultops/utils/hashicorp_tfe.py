...
def tfe_resource(
    ...
    if _resource_details_response.status_code == 200:
        ...
        _resource_create_data: Dict[str, Any] = {"data": {"type": resource_type, "attributes": {}}}
        ...
        _tfe_resource_create_response = requests.post(
            resource_url, timeout=30, headers=headers, json=_resource_create_data
        )
    ...
    if not result[f"{resource_type}_{resource_name}_created"]:  # pylint: disable=too-many-nested-blocks
        if resource_attributes and len(resource_attributes) > 0:
